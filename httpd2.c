...
static void trap_signal(int sig, sighandler_t handler)
{
	...
	if(sigaction(sig, &act, NULL) < 0) log_exit("sigaction() failed: %s", strerror(errno));
}
...
static struct HTTPRequest* read_request(FILE *in)
{
	...
	if(req->length != 0){
		if(req->length > MAX_REQUEST_BODY_LENGTH) log_exit("reqeust body too long");
...
static void read_request_line(struct HTTPRequest *req, FILE *in)
{
	...
	*p++ = '\0';
	...
	if(strncasecmp(p, "HTTP/1.", strlen("HTTP/1.")) != 0) log_exit("parse error on request line (3): %s", buf);
	p += strlen("HTTP/1.");
...
static void do_file_response(struct HTTPRequest *req, FILE *out, char *docroot)
{
	...
	if(strcmp(req->method, "HEAD") != 0){
		...
		for(;;){
			...
			if(n == 0) break;
			if(fwrite(buf, 1, n, out) < n) log_exit("failed to write to socket: %s", strerror(errno));
		}
		close(fd);
	}
	fflush(out);
